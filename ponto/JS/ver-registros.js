document.addEventListener('DOMContentLoaded', function() {
        ...
        if (registrosDoUsuario.length === 0) {
            ...
            registrosDoUsuario.forEach((registro, index) => {
                ...
                linha.innerHTML = `
                    ...
                    <td>${registro.hora}</td>
                    <td>
                        ${registro.foto && registro.foto.length > 0 ? registro.foto.map((foto, i) => `
                            <img src="${foto}" style="width: 50px;">
                            <a href="${foto}" download="foto_${registro.tipoPonto}_${i + 1}.jpg">Baixar</a>
                        `).join('') : 'Nenhuma foto'}
                    </td>
                    <td>
    ...
    window.editarCampo = function(index, campo) {
        ...
        document.getElementById('editar-data-group').style.display = campo === 'data' ? 'block' : 'none';
        document.getElementById('editar-tipo-ponto-group').style.display = campo === 'tipoPonto' ? 'block' : 'none';
        document.getElementById('editar-hora-group').style.display = campo === 'hora' ? 'block' : 'none';
        document.getElementById('editar-foto-group').style.display = campo === 'foto' ? 'block' : 'none';

        // Preenche o formulário com os dados atuais
        if (campo === 'data') {
            document.getElementById('editar-data').value = registro.data.split('T')[0];
    ...
    editarForm.addEventListener('submit', function(event) {
        ...
        if (campoEditando === 'data') {
            registrosDoUsuario[registroEditando].data = document.getElementById('editar-data').value;
        } else if (campoEditando === 'tipoPonto') {
            registrosDoUsuario[registroEditando].tipoPonto = document.getElementById('editar-tipo-ponto').value;
        } else if (campoEditando === 'hora') {
            registrosDoUsuario[registroEditando].hora = document.getElementById('editar-hora').value;
